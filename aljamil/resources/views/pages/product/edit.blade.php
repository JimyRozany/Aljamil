...
@section('content')
    <div class="w-96 mt-4">
        <form class="space-y-4 w-full" action="{{ route('product.update', $product->id) }}" method="POST"
            enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <input type="text" class="border-b-2  w-full px-2 py-1 outline-none focus:border-b-gray-800 " placeholder="Name"
            ...
            <select class="border-2 border-gray-200 rounded px-3 py-2 w-full" name="select_company">
                ...
                @foreach ($companies as $company)
                    <option value="{{ $company->id }}"
                        selected="{{ $product->company_id === $company->id ? true : false }}">{{ $company->name }}</option>
                @endforeach
            </select>
